...
startPhilosophers(N,Nodes,Pids) ->
	...
	startPhilosophers(N - 1,Nodes,[spawn(Ip1,fun() ->  philosopher_cup:philosopher_init() end)|Pids]).
...
philosopher_loop(LeftPid, RightPid, HasLeft, HasRight, HasCup, IsThirsty, State) ->
	receive
		...
		bong ->
			...
			advance(LeftPid, RightPid, HasLeft, HasRight, HasCup, yes, State);
		left_check ->
			if
				HasLeft == no ->
					Msg = success;
		...
		cup ->
			if
				(HasCup == yes) and (IsThirsty == no) ->
					erlang:display({self(), "GIVING AWAY CUP"}),
					RightPid ! success,
...
step(success, LeftPid, RightPid, _, _, _, yes, State) ->
	erlang:display({self(),got_cup}),
	erlang:display({self(),done_drinking}),
	philosopher_loop(LeftPid, RightPid, yes, no, yes, no, State);
